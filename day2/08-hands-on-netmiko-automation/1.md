...
<details>
  ...
  R1 is the device that responds since it's configured with the 10.1.1.201/24 IP address.
 </details>

<details>
  <summary>Click to reveal the answer for (b)</summary>
      
  The username and password were exported as environment variable earlier in Step 1.
 </details>

<details>
  <summary>Click to reveal the answer for (c)</summary>
      
  ``os`` is one of Python's most commonly used standard library modules for anything involving the file system or OS interaction. Without importing ``os``, you wouldn't have access to operating system-level functions, like reading environment variables.
 </details>
...
<details>
  ...
  Yes, for example, if you try to connect to an unreachable IP address, you should see the following message:
...
Device settings: cisco_ios **10.1.1.212:22**
 </details>
...
2. Complete the challenge by filling in the blanks for the task items D - F.
 ...
 <details>
  ...
  You need to specify the device type as ``cisco-ios``. The username was defined as ``USERNAME`` and the password was defined as ``PASSWORD`` earlier in Step 1.
 </details>

 <details>
  <summary>Click to reveal the answer for Task E</summary>
     
  You need to specify the ``ConnectHandler`` function to initiate the connection and specify the ``disconnect`` method to close the connection.
 </details>
...
3. Complete the challenge by filling in the blanks for the task items G - I.
   ...
   You need to import the ``ConnectHandler`` class and the ``cisco_devices`` function.
   </details>

    <details>
    <summary>Click to reveal the answer for Task H</summary>
     
      You need to add ``"""`` at the start and end of the documentation comment.
     </details>
...
6. Consider the following questions and check your understanding of the automation we just performed:
    ...
    - a) How did we get the username, password, and other device details read inside the `netmiko_function.py` script without using environmental variables in that script?
...
2. Complete the challenge by filling in the blanks for the task items J - L.
    ...
    You need to specify the ``config_commands`` method.
    </details>
...
5. Consider the following questions and check your understanding of the automation we just performed:
    ...
    - c) How could we modify one of the previous task's scripts to verify that our configuration changes from Task 5 were applied successfully?
        ...
        <summary>Click to reveal the answer for (a)</summary>
         ...
         The ``send_command`` method is used to execute commands that display information without changing the device configuration. The ``send_config_set`` method is used Used to execute commands that change the device configuration.
        </details>
        
        <details>
        <summary>Click to reveal the answer for (b)</summary>
         
         To ensure that the configuration changes are saved to NVRAM in the startup-config.
        </details>
