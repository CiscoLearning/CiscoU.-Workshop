...
1. Access your Jumphost by following the [HTD Lab Portal](https://htdlab.cisco.com) link. Log in, spin up your lab pod and click the Jumphost shortcut image. This will launch a desktop environment within your browser (Chrome preferred).
...
3. When VSCode launches, you will be presented with the Welcome Screen.  If the Explorer pane is not already displaying the CISCOU.-WORKSHOP folder content follow the following steps: Click on the `Explorer` icon on the top of the left-hand sidebar, then select `Open Folder`.  Navigate to `~/code/CiscoU.-Workshop/` and click **Open**.  This will display a folder structure of all files within the workshop.

    ![find folder](./images/step0-3.gif)

4. Everything within the web browser will function as a normal Ubuntu computer, and you have access to all of the tools that are installed.  To copy and paste between your local system and the Ubuntu jumphost, you'll need to use a keyboard shortcut to pop out the host menu, which will vary based on your host operating system:

    * **Windows/Linux:** `ATL+CTRL+SHIFT`
    * **MacOS:** `OPTION+CTRL+SHIFT`

    The resulting window that appears will allow you to control several options within Guacamole, however, the most important is the box at the top, which controls the remote clipboard.  Any text that you wish to copy from your host to the remote Ubuntu workstation can be placed here, after which a `Paste` action will place the text at the cursor location.

    For text to be copied *from* the Ubuntu host, perform a standard `Copy` action from keystrokes or right-click menu.  This will place the text in the remote clipboard, at which point you will need to activate the menu, copy it from the clipboard and then paste to your local system at the desired location.

    ![Guacamole remote clipboard](./images/step0-4.png)

    > Note: You *may* be able to copy/paste directly to/from your local system to the Ubuntu workstation without having to open the remote clipboard.  However, this is system, plugin, and browser dependent.  If it does not work, then use the above steps to copy and paste between hosts.
...
4. Create an `if` conditional that prints a message if `device_details["alerts"]` is `True`, or `else` prints some other message.
    ...
    <br><pre>device_details["alerts"] = 3</pre>
    This is due to the fact that we're checking to see if the key in question (<b><i>alerts</i></b>) contains a value.  Any value could be used in place of <b><i>3</i></b> for this to work.
    </details>
    <br>
...
> In the Ubuntu jumphost for this hands-on Lab, your virtual environment should already be created and activated for you.  You should have already seen your terminal prompt in steps above prepended with `(.venv)`.  If you do not see this, please raise your hand and alert your proctors.
